...
class Bot():
    ...
    def vet(self):
        ...
        bot = Client("BotSession", self.api_id, self.hash, bot_token="5554216479:AAE5fMHhD3s0Oyt1sYJwuD4Ucg5Oy3g5GoA")
                ...
                try:
                    ...
                    if message.text[0] == "+":
                        if message.text[-1] == '+':
                            ...
                            file.close()
                        bot.delete_messages(message.chat.id, message.id)
                    bot.send_message(message.chat.id, text="Оберіть те що вам потрібно", reply_markup=self.keyboard)
                except ValueError:
                    pass
